...
//Tüm ürünlerin yüklenme aşaması
function loadItems(){
    fetch('http://fakestoreapi.com/products')
    .then(response => response.json())
    .then((data) => {
        //Tüm Ürünleri bir değişkende sakla.
...
//Filtreleme işlemi fonksiyonu
function filterItems(){
    // Girilen fiyat ve puan aralıklarını al
    ...
    window.allProducts.forEach((product) => {
        ...
        if ((isNaN(minPrice) || product.price >= minPrice) &&
            ...
            (isNaN(minRate) || product.rating.rate >= minRate) &&
            (isNaN(maxRate) || product.rating.rate <= maxRate)) {
            createProductItem(product);
        }
    });
}

//Kategori filtreleme işlemini gerçekleştir
